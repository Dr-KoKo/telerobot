...
**Status**: Implemented; automated validation passed (2026-07-22); external playtest outcome criteria remain to be measured

**Input**: 사용자 설명 — 단일 플레이어 3D 좀비 거점 방어 슈터 「텔레 로봇팀, 출격하라」의 MVP 수직 슬라이스 기능 명세. 기본 3인칭과 선택 가능한 1인칭 시점에서 빠른 좀비가 점진적으로 개방되는 경로를 통해 중앙 거점을 공격하는 동안, 플레이어가 직접 사격하면서 강력하지만 배터리에 제약받는 로봇 아군을 지휘하는 10~15분 분량의 핵심 게임플레이 루프를 검증한다.
...
플레이어는 게임을 시작하면 "텔레 로봇팀, 출격하라." 무전을 듣고 중앙 거점을 지키는 야전 지휘관이 된다. 북쪽 도로(North Road)로 빠른 러너(Runner) 좀비가 몰려오고, 플레이어는 기본 3인칭 또는 전환 가능한 1인칭 시점에서 이동·조준하며 돌격소총으로 직접 사격한다. 두 대의 해태(Haetae) 전투 로봇이 자동으로 좀비를 탐지·추격·처치한다. 거점에 도달한 좀비는 거점 HP를 깎고, 거점이 파괴되거나 플레이어가 사망하면 패배한다. 1개 페이즈만으로도 "빠른 좀비 + 압도적으로 강한 해태 로봇 + 중앙 거점 방어"라는 핵심 환상을 체험할 수 있는 최소 단위 슬라이스다.
...
8. **Given** 해태 로봇이 좀비를 처치했고 탐지 반경 안에 다른 좀비가 있음, **When** 현재 교전이 끝나면, **Then** 해태는 거점으로 즉시 복귀하지 않고 가까운 후속 표적을 연속 교전한다.

---

### User Story 2 - 해태 로봇 지휘와 배터리 전략 (Priority: P1)

플레이어는 단순한 생존자가 아니라 로봇 부대의 지휘관이다. 해태 로봇에게 **거점 사수(Defend Position)·경로 순찰(Patrol Route)·기지 복귀(Return to Base)** 3가지 명령을 내릴 수 있다. 모든 행동은 배터리를 소모한다(대기 0.3/초, 순찰 0.8/초, 전투 2.5/초). 배터리가 낮아지면 로봇이 느려지고 약해지며, 0이 되면 기능을 멈춘다. 로봇이 기지 충전 반경에 들어와 교전 대상이 없으면 별도의 명령 없이 자동으로 충전(4/초)되고, 충전 중 새 적을 탐지하면 충전을 중단하고 기지 방어에 복귀한다. 따라서 플레이어는 수동 충전 메뉴 대신 `기지 복귀` 시점을 결정한다.

**Why this priority**: 배터리·충전·명령은 본 게임의 핵심 차별 요소다. 이 메커닉이 없으면 평범한 슈터에 불과하며, "해태는 강한데 충전 타이밍을 놓치면 거점이 위험해진다"는 검증 목표 자체가 성립하지 않는다.

**Independent Test**: 명령 메뉴로 3개 명령을 내리고, 명령별 배터리 소모율 차이를 확인하고, 배터리 11~30% 구간에서 이동 -15%·공격속도 -10% 저하가 적용되고, 배터리 0에서 로봇이 정지하며, 기지 충전 반경에 진입하면 4/초로 자동 회복되는지로 독립 검증한다.

**Acceptance Scenarios**:

1. **Given** 해태 로봇이 선택된 상태, **When** 명령 메뉴를 열면, **Then** Defend Position·Patrol Route·Return to Base 3개 명령만 제공되고 별도 Charge 명령은 없다.
...
6. **Given** 배터리가 최대치 미만인 로봇이 교전 대상 없이 기지 충전 반경에 있음, **When** 1초가 경과하면, **Then** 별도 명령 없이 배터리가 4 회복되며 충전 중 새 적을 탐지하면 충전을 중단하고 방어에 복귀한다.
7. **Given** 플레이어가 어느 한쪽 경로 방어가 시급한 상황, **When** 로봇에 기지 복귀를 내릴지 전투를 유지할지 결정하면, **Then** 자동 충전(전력 회복)과 전투 공백 사이의 트레이드오프가 발생한다.
8. **Given** 로봇이 Depleted(배터리 0)로 Disabled됨, **When** 일정 시간(MVP 잠정 기본값 5초) 경과 후, **Then** 로봇은 Recovery 상태로 전환되어 배터리를 0.5/초(잠정)로 회복하고, 이동 가능 임계값(배터리 5, 잠정) 도달 시 자동으로 기지로 복귀하며, Recovery·복귀 중에는 공격할 수 없다.
...
### User Story 6 - 플레이테스트 진입·조작·설정 (Priority: P2)

플레이어는 Unity 편집기나 프로젝트 구조를 몰라도 시작 화면에서 게임에 진입하고, 자신에게 맞는 조작·화면·음량 설정을 저장해 반복 플레이할 수 있다. 전투 중에는 1인칭·3인칭 시점을 전환하고 점프·달리기를 사용할 수 있으며, 일시정지 화면에서 설정을 바꾸거나 세션을 다시 시작하고 시작 화면으로 돌아갈 수 있다.

**Why this priority**: 외부 플레이테스트에서 설치 뒤 실제 핵심 루프까지 도달하는 마찰을 줄이고, 시점·감도 차이 때문에 게임 밸런스 피드백이 왜곡되는 것을 막는다.

**Independent Test**: 시작 화면에서 설정을 저장한 뒤 게임을 시작하고, 저장한 기본 시점·감도·음량·화면 설정이 적용되는지, 전투 중 시점 전환·점프·달리기·일시정지가 동작하는지, 일시정지/결과 화면에서 재시작 또는 시작 화면 복귀가 가능한지로 독립 검증한다.

**Acceptance Scenarios**:

1. **Given** 독립 실행 게임을 시작함, **When** 첫 화면이 로드되면, **Then** 게임 시작·설정·게임 종료를 선택할 수 있는 시작 화면이 표시된다.
2. **Given** 플레이어가 감도·음량·해상도·전체 화면·기본 시점을 변경함, **When** 저장하고 새 게임을 시작하면, **Then** 설정이 로컬에 유지되고 게임에 적용된다.
3. **Given** 전투 중인 플레이어, **When** 시점 전환·점프·달리기 입력을 사용하면, **Then** 1인칭과 3인칭이 전환되고 접지 상태에서 점프하며 기본 이동보다 빠르게 달린다.
4. **Given** 전투 중인 플레이어, **When** 일시정지를 열면, **Then** 게임 시간이 멈추고 계속하기·설정·다시 시작·시작 화면 복귀를 선택할 수 있다.
...
- **Disabled 로봇의 복귀 경로**: 배터리 0인 로봇은 이동할 수 없는데 어떻게 기지로 복귀하는가? (Assumptions 참조 — 자동 미세 회복(Recovery) 후 이동 가능 임계값 도달 시 복귀.)
...
- **충전 중 거점 피격**: 로봇이 기지에서 자동 충전 중일 때 좀비가 거점에 접근하면? (탐지 반경 안의 유효한 기지 위협은 배정 경로와 무관하게 충전을 중단시키고 Engage로 전환된다. 탐지 반경 밖 위협은 로봇이 인지하지 못하므로 플레이어가 직접 방어해야 한다.)
...
- **위협 예산과 목표 마릿수 불일치**: 위협 예산(Phase 1=40)과 목표 마릿수(러너 18~24) 사이의 구성 규칙. (Assumptions 참조 — 예산은 상한, 페이즈 학습 목표에 맞는 구성을 우선한다.)
...
- Q: 로봇 명령(3가지)의 대상 지정 방식은(개별 vs 전체)? → A: 두 해태 로봇을 개별 선택해 명령하거나, '전체 선택' 토글로 두 대에 동시 명령할 수 있다(개별 지휘와 일괄 지휘를 모두 지원). 명령은 로봇 단위로 적용된다.

### Session 2026-07-22

- Q: 해태 로봇 충전을 수동 명령으로 유지해야 하는가? → A: 별도 `Charge` 명령을 제거하고, `Return to Base`로 기지에 복귀한 뒤 유효한 표적이 없을 때 자동 충전한다. 명령 메뉴는 정확히 3개만 제공한다.
- Q: 자동 충전 중 기지 주변 적이 나타나면 계속 충전해야 하는가? → A: 탐지 반경 안의 기지 위협은 배정 경로와 무관하게 충전을 중단시키고 `Engage`로 전환하며, 해당 표적이 무효가 될 때까지 유지한다.
- Q: 현재 플레이테스트 빌드에 추가된 시점·이동·설정 기능을 MVP 명세에 포함할 것인가? → A: 1인칭/3인칭 전환, 점프·달리기, 시작 화면, 저장 설정, 일시정지를 현재 MVP 플레이테스트 접근성 범위로 명세에 반영한다.
...
- **FR-007**: 시스템은 게임 시작·설정·게임 종료를 제공하는 시작 화면을 제공해야 한다(MUST).
- **FR-008**: 시스템은 마우스 감도·전체 음량·효과음 음량·해상도·전체 화면·기본 시점을 플레이어가 조정하고 로컬에 저장할 수 있게 해야 한다(MUST).
- **FR-009**: 시스템은 전투 중 게임 시간을 멈추는 일시정지와 계속하기·설정·세션 재시작·시작 화면 복귀를 제공해야 한다(MUST).

#### 플레이어 · 무기 (Player & Weapon)

- **FR-010**: 시스템은 기본 3인칭 시점의 플레이어 이동과 조준을 제공해야 한다(MUST).
- **FR-010A**: 플레이어는 전투 중 1인칭·3인칭 시점을 전환할 수 있어야 하며(MUST), 두 시점은 서로 구별되는 시야와 캐릭터 표시를 제공해야 한다(MUST).
- **FR-010B**: 플레이어는 접지 상태에서 점프하고 입력을 유지해 기본 이동보다 빠르게 달릴 수 있어야 한다(MUST).
- **FR-010C**: 3인칭 카메라는 벽·지형과 충돌할 때 플레이어와 장애물 사이로 자동 보정되어 장애물을 뚫고 보이지 않아야 한다(MUST).
- **FR-011**: 플레이어는 기본 돌격소총으로 직접 사격할 수 있어야 한다(MUST).
- **FR-011A**: 왼쪽 클릭을 한 번 누르면 1발을 발사하고, 누르고 있으면 0.12초 간격으로 탄약이 소진될 때까지 연속 발사해야 한다(MUST).
- **FR-011B**: 매 발마다 상향 0.70~1.15도와 좌우 최대 0.32도 범위에서 무작위 카메라 반동을 적용하고, 반동은 데이터 구동 회복 속도로 원위치해야 한다(MUST).
...
- **FR-016A**: 시스템은 조준점, 일반 명중·헤드샷 구분, 재장전 진행, 탄약 부족·근처 보급지 안내, 피격 방향을 전투 중 식별 가능한 피드백으로 제공해야 한다(MUST).
...
- **FR-035**: 거점 내부 또는 인접 위치에 데이터 구동 충전 반경이 존재해야 하며(MUST), 해태가 그 반경 안에 있고 배터리가 최대치 미만이며 교전 대상이 없으면 자동 충전을 시작해야 한다(MUST). 충전 중에는 배정 경로와 무관하게 탐지 반경 안의 기지 주변 적을 확인하고(MUST), 새 적을 탐지하면 충전을 중단한 뒤 그 표적이 무효가 될 때까지 유지하며 기지 방어에 복귀해야 한다(MUST).
- **FR-036**: 메디컬 로봇 투입 이후, 거점 근처에 메디컬(치료) 구역이 존재해야 한다(MUST).
- **FR-037**: 탄약 보급은 두 곳에 존재해야 한다 — 거점 내부의 안전 보급지점 1곳과, 거점 외부 또는 교전 지역 근처의 위험 보급지점 1곳(MUST). 보급 진입은 지면 수평 거리로 판정하고(MUST), 진행 중에는 작은 경계 흔들림으로 취소되지 않도록 데이터 구동 이탈 여유를 적용해야 한다(MUST).
...
- **FR-053**: 각 페이즈는 학습 목표에 적합한 좀비 구성을 스폰해야 한다(MUST). 권장 구성: Phase 1은 러너 18~24; Phase 2는 총 30~39마리(브루저 2~3 포함); Phase 3는 총 47~55마리(브루저 2~3, 리퍼 3~4 포함).
...
- **FR-078**: 시스템은 좀비를 탐지·추격·공격하는 로봇 AI를 제공해야 하며(MUST), 해태가 표적을 처치했을 때 탐지 반경 안에 유효한 다른 좀비가 있으면 가장 가까운 적을 즉시 후속 표적으로 획득해야 한다(MUST). `거점 사수` 중 후속 표적은 배정 경로와 달라도 허용하되 기존 거점 방어 반경을 벗어나서는 안 된다(MUST).
- **FR-079**: 해태 로봇은 다음 9개 상태를 지원해야 한다(MUST): `Standby`, `Patrol`, `Engage`, `LowBattery`(표기: Low Battery), `ReturnToCharge`(표기: Return to Charge), `Charging`, `Disabled`, `Recovery`, `Destroyed`.
...
- **FR-085**: 로봇 명령 메뉴는 다음 3개 명령만 제공해야 한다(MUST): 거점 사수(Defend Position), 경로 순찰(Patrol Route), 기지 복귀(Return to Base). 별도 충전(Charge) 명령과 그 외 명령을 포함하지 않아야 한다(MUST NOT).
...
- **FR-091**: 배터리 상태 구간은 다음과 같아야 한다(MUST): Normal 31~100, Low Power 11~30, Critical 1~10, Depleted 0, Charging(기지에서 자동 충전 중).
...
- **FR-097**: 기지에서 `Charging` 상태를 유지하는 로봇은 충전과 공격을 동시에 수행할 수 없어야 한다(MUST). 단, FR-035의 기지 위협을 탐지하면 먼저 충전을 중단하고 `Engage`로 전환한 뒤 전투에 참여해야 한다(MUST).
...
- **FR-113**: 업그레이드 후보는 다음 9종이어야 한다(MUST):
  ...
  6. **긴급 방벽**: 페이즈 시작 시 각 경로에 1회용 방벽을 생성하며, 각 방벽은 해당 경로의 거점 진입 방향을 가로지르도록 회전
...
- **플레이어(Player)**: 야전 지휘관. 속성 — HP 100, 보유 무기(돌격소총), 수류탄 보유량, 위치, 현재 시점(1인칭/3인칭), 접지·달리기 상태. 사망 시 패배.
...
- **해태 전투 로봇(Haetae Combat Robot)** ×2: HP 300, 최대 배터리 100, 근접 돌진/물기, 상태(`Standby`/`Patrol`/`Engage`/`LowBattery`/`ReturnToCharge`/`Charging`/`Disabled`/`Recovery`/`Destroyed`), 현재 명령. HP 0 시 현재 페이즈 동안 Destroyed(잔해), 다음 페이즈 시작 시 복원(FR-081).
...
- **기지 충전 구역(Base Charging Zone)**: 거점 중심 반경 6m. 유효한 전투 표적이 없는 로봇이 반경 안에 있으면 별도 명령 없이 배터리 4/초 회복. `Charging`과 공격은 동시에 수행하지 않지만, 탐지 반경 안의 기지 위협은 충전을 중단시키고 `Engage` 전환을 유발한다.
- **플레이어 설정(Player Settings)**: 마우스 감도, 전체/효과음 음량, 해상도, 전체 화면, 기본 시점. 로컬에 저장되어 다음 세션에 적용된다.
...
- **SC-011**: 평균적으로 1회 플레이당 최소 2회 이상 로봇의 기지 자동 충전이 시작된다.
...
- **Disabled 로봇 회복(Recovery) 메커니즘**: 입력은 "충전 또는 회복으로 가용 상태가 되기 전까지 이동·공격 불가"라고만 명시한다. 0 배터리 로봇이 스스로 기지로 갈 수 없는 모순을 해소하기 위해, Depleted 로봇은 Disabled 상태로 제자리에 정지한 뒤 **느린 미세 자동 회복(Recovery)**으로 소량의 배터리를 시간에 따라 회복하고, 이동 가능 최소 임계값에 도달하면 자동으로 기지에 복귀한다고 가정한다. **MVP 잠정 기본값**: Depleted 진입 후 5초간 Disabled 유지 → 이후 Recovery 상태에서 0.5/초 회복 → 배터리 5 도달 시 자동 Return to Base 수행 → 기지 진입 후 자동 충전 → Recovery·복귀 중에는 공격 불가. (수치는 밸런싱 대상 잠정값이며, US2 합격 시나리오 8은 이 값으로 검증한다.)
...
- **로봇 개별 지휘**: 두 해태 로봇은 개별 선택 가능하며, 명령은 로봇 단위로 적용된다(FR-087). 명령 퀵메뉴는 개별 선택된 로봇 또는 '전체 선택' 토글로 지정한 두 로봇 모두에 3개 명령을 발행한다.
- **경로 순찰 대상 지정**: Patrol Route 명령 시 플레이어가 순찰할 경로를 지정한다고 가정한다(개방된 경로 중 선택). Defend Position은 지정한 지점/경로를 사수한다.
- **위협 예산 vs 목표 마릿수**: 위협 예산(40/60/80)을 **하드 스폰 상한**으로 보고, 그 한도 내에서 각 페이즈 학습 목표에 맞는 현재 FR-053 구성과 특수적 최소 수량(Phase 2 브루저 2, Phase 3 브루저 2 및 리퍼 3)을 보존한다고 가정한다. 현재 조정 범위는 모두 예산 안에 들어온다. 이후 밸런스 조정으로 상한을 넘는 경우에는 특수적 최소 수량을 먼저 보존하고 `trimOrder`에 따라 러너부터 줄여 예산 내로 맞춘다.
...
- **업그레이드별 세부 적용 규칙**: (1) 해태 돌진 강화 — "첫 돌진"은 한 교전(타깃 인게이지)당 첫 돌진 1회에 적용된다(전투 진입마다 1회). (2) 긴급 방벽 — 페이즈 시작 시 각 개방 경로의 거점 진입 길목 1곳에 1회용으로 생성되며, 마지막 경로 구간의 진입 방향에 직각으로 놓이고 누적 피해로 1회 파괴된다(방벽 HP·지속 시간은 밸런싱 잠정값). (3) 관통탄 — 탄환이 러너 1마리를 추가 관통하되, 두 번째 표적이 브루저/리퍼이면 관통하지 않고 정지한다(러너 한정). (4) 고효율 배터리 — 배터리를 가진 로봇(해태)에만 유효하며 메디컬 로봇은 배터리가 없어 무효다.
- **업그레이드 제시 정책(clarify 확정)**: 9개 업그레이드 정의는 모든 보상 단계에서 동일한 전역 후보 풀로 유지된다(FR-115의 "페이즈별 보상풀에서 후보를 제거하지 않는다"와 동일). 단, **같은 세션에서 이미 선택한 업그레이드 id는 이후 보상 단계의 3택 제시에서 제외**된다. 이는 "페이즈별 pool gating 없음"(유지)과 "세션 내 이미 선택한 id 재등장 없음"(신규)을 분리한 것으로 FR-115와 충돌하지 않는다. MVP에서 업그레이드는 **중첩(stacking) 없음** — 각 업그레이드는 세션당 최대 1회 선택된다(세션 전체 최대 2회 선택, FR-112).
- **배터리 경고 문구 대상(clarify 확정)**: MVP에서는 배터리 경고 무전을 스펙 문구 "해태 1호, 배터리 위험."(FR-130) 하나로 유지하고, 실제로 위험한 로봇(1호/2호)의 식별은 HUD의 로봇 배터리 위젯/robotId 표시로 구분한다고 가정한다. 로봇별 개별 음성 라인은 MVP 범위 밖(향후 VO 교체 시 검토).
- **메디컬 로봇 표적 정책(clarify 확정)**: 좀비 표적 우선순위의 "로봇"은 **해태 전투 로봇만** 의미하며(리퍼는 해태 카운터), 메디컬 로봇은 어느 좀비의 표적 우선순위에도 포함되지 않는다. 메디컬 로봇은 좀비가 상위 표적으로 이동하다 인접했을 때 **부수적으로만** 피해를 입어 파괴될 수 있다(FR-107 "좀비가 도달하면"). MVP에서 메디컬 로봇을 능동적으로 추적하는 좀비는 없다.
- **타깃 플랫폼·입력·관점 세부**: 직접 사격·로봇 명령·1인칭/3인칭 전환·점프·달리기가 가능한 표준 컨트롤 스킴을 전제하되, 구체 엔진·플랫폼·입력 장치 구현은 명세하지 않는다.
