...
- **위협 예산 vs 목표 마릿수**: 위협 예산(40/60/80)을 **하드 스폰 상한**으로 보고, 그 한도 내에서 각 페이즈 학습 목표에 맞는 구성(특수적 최소 수량 — Phase 2 브루저 2, **Phase 3 브루저 2 및 리퍼 3**(Phase 3는 러너·브루저·리퍼가 모두 섞이는 종합 국면이므로 브루저 최소 2를 확정 — clarify 확정 사항) — 과 적 종류 믹스)을 우선 보존한다고 가정한다. FR-053의 권장 총 마릿수 상단이 예산을 초과하는 구간(예: Phase 3 총 72~75마리, Phase 2 총 50마리+브루저 3 → 비용 84~90/62 > 예산 80/60)에서는 예산을 우선하고 가장 싼 유닛(러너) 수를 줄여 예산 내로 맞춘다. 즉 충돌 시 예산이 마릿수를 이기되, 특수적 최소 수량은 보존한다.
