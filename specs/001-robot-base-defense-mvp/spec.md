...
- **FR-079**: 해태 로봇은 다음 상태를 지원해야 한다(MUST): Standby, Patrol, Engage, Low Battery, Return to Charge, Charging, Disabled, Recovery.
...
- **해태 전투 로봇(Haetae Combat Robot)** ×2: HP 300, 최대 배터리 100, 근접 돌진/물기, 상태(Standby/Patrol/Engage/Low Battery/Return to Charge/Charging/Disabled/Recovery), 현재 명령.
...
- **페이즈(Phase)**: 번호(1~3), 개방 경로, 위협 예산(40/60/80), 좀비 구성, 클리어 상태, 목표 지속 시간.
- **위협 예산(Threat Budget)**: 페이즈 스폰 한도. 비용 — 러너 1, 브루저 5, 리퍼 4.
...
- **로봇 개별 지휘**: 두 해태 로봇은 개별 선택 가능하며, 명령은 로봇 단위로 적용된다고 가정한다. 명령 퀵메뉴는 선택된 로봇(또는 전체)에 4개 명령을 발행한다.
